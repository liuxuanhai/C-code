...
char menu(void)
{
	...
	puts("a) add a pet(增)        l) show list of pets");
	puts("n) number of pets       f) find pets(查)");
	puts("d) delete a pet(删)     q) quit");
...
void droppet(Tree * pt)
{
	...
	uppercase(temp.petkind);
	printf("%s the %s ", temp.petname, temp.petkind);
	if (DeleteItem(pt, &temp))
		printf("is droopen from the club.\n");
	else

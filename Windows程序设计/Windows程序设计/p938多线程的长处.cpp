// 1.	执行的速度比单线程的快(单线程运用的是Timer给每个窗口分配时间来处理事情, 但是如果程序处理完了, 还没有进行下次分配, 此时CPU处于闲置状态)
// 2.	在移动窗口改变窗口大小的时候, 程序依旧在输出, 而在单线程中会停止输出
#include <windows.h> 
#include <math.h> 
#include <process.h> 
typedef struct 
...
{ 
	...
	while (!pparams->bKill)	// 每次循环时检查bKill字段是TRUE还是FALSE, 如果是FALSE, 格式化数字, 获取设备环境句柄, 调用TextOut显示数字
	{ 
		if (iNum < 0) 
			iNum = 0 ; 
	...
	_endthread () ;		// 调用这个函数结束执行, 这不是必须的, 因为在线程函数退出后, 线程会被自动销毁, 但是, 如果线程处于一个复杂的处理过程中, 调用_endthread()是很有用的
} 
LRESULT APIENTRY WndProc1 (  HWND  hwnd,  UINT  message,  WPARAM  wParam,  LPARAM lParam) 
{ 
	static PARAMS params ;	// 唯一的局部变量
	switch (message) 
	{ 
	case WM_CREATE: 
		params.hwnd = hwnd ; 
